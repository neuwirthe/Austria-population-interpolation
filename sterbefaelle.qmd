...
#  mutate(Alter=ifelse(str_sub(`C-ALTERGR65-0`,12,12)==1,"<65","65+")) %>%
 ...
 sterbedaten_5y_tmp |>
   ...
   mutate(Alter=factor(Alter,levels=Alter)) ->
   alter_altersgruppe
 sterbedaten_5y_tmp |>
   select(-Alter) |>
   left_join(alter_altersgruppe) |>
   select(Jahr:Altersgruppe,Alter,everything()) ->
    sterbedaten_5y_tmp
```
```{r}
    sterbedaten_5y_tmp |>
...
datum_jahr_woche |> 
         filter((Jahr < last_year) | 
         (Woche <= last_week & Jahr == last_year)) ->
  datum_jahr_woche
```
...
```{r}
load(path(save_path,"pop_aut_bl_5y_gender_weekly.RData"))
```
...
```{r}
  pop_death_aut_bl_complete |>
  save(file=path(save_path,"pop_death_aut_bl_complete.RData"))
```
...
pop_death_aut_bl_complete |> 
 left_join(
pop_death_aut_bl_complete |>
  filter(Jahr==curr_year & Woche == 1) |>
  select(BundeslandID,Altersgruppe,Alter,Geschlecht,pop) |>
  rename(pop_curr=pop)
) |>
  mutate(Anzahl_curr=pop_curr*Anzahl/pop) ->
pop_death_aut_bl_complete
```
...
pop_death_aut_bl_complete |>
  select(-Datum) |>
  group_by(Jahr,Woche) |>
  summarise(Anzahl_curr=sum(Anzahl_curr),
            Anzahl=sum(Anzahl),
            pop=sum(pop),
            pop_curr=sum(pop_curr)) |>
  ungroup() |>
#  mutate(Anzahl=100000*Anzahl/pop) |>
#  mutate(Anzahl_curr=100000*Anzahl_curr/pop_curr) |>
  left_join(datum_jahr_woche()) |>
  filter(Jahr >= 2018) |>
  select(Datum,Anzahl,Anzahl_curr) |>
  pivot_longer(cols = starts_with("Anzahl"),
               names_to="Alterstruktur",
               values_to = "Verstorbene") |>
  mutate(Altersstruktur=ifelse(Alterstruktur=="Anzahl_curr",
                               paste("von", curr_year), "laufend")) |>
...
(pop_death_aut_bl_complete |>
  ...
  left_join(datum_jahr_woche()) |>
  filter(Jahr >= 2018) |>
  select(Datum,Anzahl,Anzahl_curr,BundeslandID) |>
  left_join(namen_bl) |>
  pivot_longer(cols = starts_with("Anzahl"),
               names_to="Alterstruktur",
               values_to = "Verstorbene") |>
...
base_year <- 2018
pop_aut_bl_5y_gender_weekly |>
  filter(Jahr==base_year) |>
  filter(Woche==min(Woche)) |>
  select(BundeslandID,Altersgruppe,Geschlecht,pop) |>
  rename(pop_base=pop) ->
  pop_base
```
...
pop_death_aut_bl_complete |>
  ...
  mutate(Alter_65 = ifelse(Altersgruppe <= 13,"<65","65+")) |>
  filter(Alter_65 == "65+") |>
  mutate(Anzahl_65=100000*Anzahl*pop2024/pop) |>
  group_by(Jahr,Woche,Datum,Alter_65,Geschlecht) |>
...
```{r}
tote_gesamt <- read_csv2("https://data.statistik.gv.at/data/OGD_gestorbene_ext_GES_1.csv")
```
```{r}
tote_gesamt
```
