...
sheet_names <-
  list_ods_sheets(
    path(here(),"ods_data",
         "Jaehrliche_Sterbetafeln_1947_bis_2024_fuer_Oesterreich.ods")
  )
```
...
```{r}
file_exists(       path(here(),"xlsx_data",
         "Jaehrliche_Sterbetafeln_1947_bis_2024_fuer_Oesterreich.xlsx")
)
```
...
#  filter(Alter==0|Alter==20) |>
  ...
  ggplot(aes(x=Jahr,y=Lebenserwartung,color=Geschlecht,
             linetype=Alter)) +
  geom_line() +
  scale_x_continuous(breaks=c(seq(1900,2020,by=10),2024)) +
  scale_y_continuous(minor_breaks = 50:100) +
  theme_minimal() +
  scale_linetype_discrete("Lebenserwartung") +
  guides(linetype=guide_legend(reverse=TRUE)) +
  labs(title="Lebenserwartung nach Alter und Geschlecht")
